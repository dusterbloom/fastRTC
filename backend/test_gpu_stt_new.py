#!/usr/bin/env python3
"""
Test new GPU-enabled FasterWhisperGPUSTT
"""
...
async def test_gpu_stt():
    """Test GPU functionality with FasterWhisperGPUSTT."""
    logger.info("🧪 Testing GPU functionality with FasterWhisperGPUSTT...")
    ...
    try:
        ...
        # Test async transcription too
        logger.info("🎯 Testing async transcription...")
        start_time = time.time()
        
        result_async = await stt._transcribe_audio(audio_samples, target_language="en")
        
        elapsed_async = time.time() - start_time
        
        logger.info(f"✅ Async transcription completed in {elapsed_async:.3f}s")
        logger.info(f"📝 Async result: '{result_async.text}'")
...
async def main():
    """Main async function"""
    logger.info("=" * 60)
    logger.info("🧪 FastRTC GPU STT Test (New Threading Implementation)")
    logger.info("=" * 60)
